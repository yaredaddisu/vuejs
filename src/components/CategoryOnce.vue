...
function deleteCategory(item) {
  categories.value = categories.value.filter((q) => q !== item);
}

let errors = ref("");

function saveSurvey(categories) {
  loading.value = true;

  store
    .dispatch("saveCategoryOnce", {
      categories,
    })
    .then(({ data }) => {
      loading.value = false;
...
<template>
  <div class="relative w-auto my-6 mx-auto max-w-3xl dark:bg-gray-700">
    ...
    >
      ...
      >
        <h3 class="text-2xl font-semibold">Add All Categories Once</h3>
        <button
      ...
      <div class="m-10">
        <form @submit.prevent="saveSurvey(categories)">
          <CardBox
