...
const reports = computed(() => store.state.userBuyReports.data);
const loading = computed(() => store.state.userBuyReports.loading);

store.dispatch("getUserBuyReports", { search: search.value });

watch([keyword, search], ([newKeyword, newSearch], [oldKeyword, oldSearch]) => {
  store.dispatch("getUserBuyReports", {
    search: newSearch,
...
function getUserBuyReports(url = null) {
  store.dispatch("getUserBuyReports", {
    url,
    search: search.value,
    sort_field: sortField.value,
...
function openPurchaseInNewWindow(id) {
  // alert(id);
  router.push({ name: "PurchaseReports", params: { id } });
}
</script>
  ...
  <div>
    ...
    <div class="overflow-x-auto w-full">
      <table>
        ...
        <tbody>
          <tr v-for="purchase in itemsPaginated" :key="purchase.id">
            <td data-label="ID">{{ purchase.id }}</td>
            <td data-label="Creator">{{ purchase.creator }}</td>

            <td data-label="Delivery">{{ purchase.dateOfDelivery }}</td>
            <td data-label="Purchase">{{ purchase.dateOfPurchase }}</td>
            ...
            <td data-label="Detail" class="before:hidden lg:w-1 whitespace-nowrap">
              <BaseButtons type="justify-start lg:justify-end" no-wrap>
                <BaseButton
                  color="info"
                  :icon="mdiEye"
                  small
                  @click="openPurchaseInNewWindow(purchase.id)"
                />
              </BaseButtons>
            </td>
          </tr>
