...
export default {
  ...
  methods: {
    ...
    getStockIn() {
      ...
      axios
        .get("/product-stockin", {
          params: { keyword: this.keyword, search: this.search },
        })
        .then((res) => {
          this.loading = false;
...
<template>
  ...
  <LayoutAuthenticated>
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiArrowExpandUp" title="Stock In History" main>
        <BaseButton
          target="_blank"
              ...
              <table class="">
                ...
                <tbody>
                  <tr v-for="(product, index) in paginatedPurchases" :key="index">
                    ...
                    <td data-label="Previous Quantity" class="text-sky-500">
                      {{ product.previous }}
                    </td>
                    <td data-label="Stock In Quantity" class="text-yellow-500">
                      {{ product.quantity }}
                    </td>
