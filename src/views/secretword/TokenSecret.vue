 ...
 <svg
  ...
  <div  v-if="tokenValid">
    ...
    <form action="" v-on:submit.prevent="changePassword" class="w-full"><div class="">
      ...
      <label for="email-address" class="block text-sm font-semibold leading-6 text-gray-900 mt-6">New  Secret Word</label>
      <input type="password"
       id="password"
        class="mt-2 appearance-none text-slate-900 bg-white rounded-md block w-full px-3 h-10 shadow-sm sm:text-sm focus:outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-sky-500 ring-1 ring-slate-200" required="" 
         ...
         <label for="email-address" class="block text-sm font-semibold leading-6 text-gray-900 mt-6">Confirm Secret Word</label>
         <input type="password"
          id="password"
           class="mt-2 appearance-none text-slate-900 bg-white rounded-md block w-full px-3 h-10 shadow-sm sm:text-sm focus:outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-sky-500 ring-1 ring-slate-200" required="" 
             placeholder="Enter Secret Word"
            v-model="secret_confirmation">
  ...
  if(!this.errorToken){
    const data = {
      ...
      this.loading = true;
     axiosClient.post('/validate-Resettoken', data)
              .then(response => {
                this.loading = false;
             if(response.data.id){
    ...
    changePassword(){
      ...
      this.errorNewSecret = null;
      this.errorsecret_confirmation = null;
   
  
         const data = {
          secret: this.newSecret,
          secret_confirmation: this.secret_confirmation,

          user_id: this.id
         }
         this.loading = true;

        axiosClient.post('/reset-Resettoken', data)
              .then(response => {
                this.loading = false;
